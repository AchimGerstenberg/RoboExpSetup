...
String iterationstr;
String filename;
long unsigned int lastkey = 4294967295;
long unsigned int lastcompile = 4294967295;
...
void taskentry()
{
  ...
  Serial.println("writing error tasklog.txt");
...
unsigned long int setunixtime()
{
  ...
  Serial.println("unix time stamp: ");
...
void loop() {
  ...
  // autosave before 1st evaluation, after 5 min without compiling and no typing for 2 sec
  if(task == 0 && millis() - lastcompile > 300000 && millis() - lastkey > 2000)
  {
    iteration++;
    createfilename();
    safefile();
...
//        break;
        ...
        if(millis()-millisoffset - 15000 < F5timer && task != 0)    
        { 
